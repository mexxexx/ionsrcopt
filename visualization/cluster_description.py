...
def main(year, source_stability, count_breakdowns_per_cluster, num_clusters_to_visualize, print_to_file, display_metrics):
    ######################
    ###### SETTINGS ######
    ######################
    ...
    described = df.groupby(ProcessingFeatures.CLUSTER).apply(describe_cluster, features=features, weight_column=ProcessingFeatures.DATAPOINT_DURATION, oven_refills=oven_refill_ends)
    ...
    wanted_statistics = get_wanted_statistics(features, statistics) + [
            ('DENSITY', 'percentage'), 
            ('DURATION', 'in_hours'), ('DURATION', 'longest_in_hours'), ('DURATION', 'num_splits'),
            ('REFILL', 'index'), ('REFILL', 'delta_in_hours'), 
        ] 
...
def clear_refills_2018(oven_refill_ends):
    ...
    return [r for r in oven_refill_ends if not r.strftime("%Y-%m-%d %Hh") in remove_dates]
...
def calculate_oven_refill_ends(df):
    ...
    continuous_periods_starts = np.flatnonzero((times_where_oven_is_off.to_series().diff(1)).apply(timedelta_breaks, min_lenght=3600).values)
...
def describe_cluster(cluster_df, features, weight_column, oven_refills):
    ...
    index = pd.MultiIndex.from_tuples([(p, v) for p in features for v in values] + [
            ('DENSITY', 'count'), 
            ('DURATION', 'in_hours'), ('DURATION', 'longest_in_hours'), ('DURATION', 'num_splits'), 
            ('REFILL', 'index'), ('REFILL', 'delta_in_hours'), 
    ...
    duration_longest_start, duration_longest, duration_num_splits = get_cluster_duration(cluster_df, weight_column)
    ...
    if not closest_refill is None:
        refill_delta = (pd.Timestamp(duration_longest_start) - oven_refills[closest_refill]).total_seconds() / 3600
...
def get_cluster_duration(cluster_df, weight_column):
    continuous_periods_starts = np.flatnonzero((cluster_df.index.to_series().diff(1)).apply(timedelta_breaks, min_lenght=3600).values)
...
def parse_args():
    ...
    parser.add_argument('-y', '--year', default=2018, type=int, help='The year you are interested in')
...
if __name__ == "__main__":
    ...
    main(year, source_stability, count_breakdowns_per_cluster, num_clusters_to_visualize, print_to_file, display_metrics)