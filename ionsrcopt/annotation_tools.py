...
def export_for_annotation(filename, column_to_export, rows):
    ...
    for x in rows:
        filename_output = (
            filename_without_ext + "_" + str(x[0]).zfill(7) + "_" + str(x[-1]).zfill(7)
        )
        print("Saving {}...".format(filename_output))

        sub_df = df.iloc[x].copy()
        sub_df["filename"] = filename_output
        sub_df["timestamp"] = sub_df["timestamp"].apply(
            lambda timestamp: parser.parse(timestamp).strftime("%Y-%m-%dT%H:%M:%S.%f")[
                :-3
            ]
            + "Z"
        )
...
def import_annotated_data(filenames=[], dirname=""):
    ...
    for f in filenames:
        ...
        groups = {
            "filename": groups[0],
            "start_index": int(groups[1]),
            "end_index": int(groups[2]),
        }
        ...
        if (
            previous_groups
            and previous_groups["filename"] == groups["filename"]
            and previous_groups["end_index"] == (groups["start_index"] - 1)
        ):
